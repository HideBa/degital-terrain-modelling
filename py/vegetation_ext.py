...
class VegetationExtractor:
    ...
    def _find_trees(self, las, number_of_returns=2):
        trees = []
        ...
        for cluster in clusters:
            ...
            if np.any(num_returns > number_of_returns):
    ...
    def find_first_return_of_trees(self, number_of_returns=2):
    ...
    def rasterize(self, file_path, cell_size=0.5, nodata=-9999):
        grid_points = np.array(self._to_gridded_points(cell_size))
    ...
    def _to_gridded_points(self, cell_size, nodata=-9999):  # meter
        bbox = self._bbox()
        rows = []
        y = bbox[1]
        while y < bbox[3]:
    ...
    def _bbox(self):
        minx, maxx = np.min(self.veg_first_returns.x), np.max(self.veg_first_returns.x)
        miny, maxy = np.min(self.veg_first_returns.y), np.max(self.veg_first_returns.y)
